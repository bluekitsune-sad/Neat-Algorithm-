...
STAT_FONT = pygame.font.SysFont(cog.FONT, 50)
END_FONT = pygame.font.SysFont(cog.FONT, 70)
DRAW_LINES = False

WIN = pygame.display.set_mode((cog.WIN_WIDTH, cog.WIN_HEIGHT))
pygame.display.set_caption(cog.GAME_NAME)
...
def draw_window(win, birds, pipes, base, score, gen, pipe_ind):
    ...
    win.blit(score_label, (cog.WIN_WIDTH - score_label.get_width() - 15, 10))
...
def eval_genomes(genomes, config):
    ...
    while run and len(birds) > 0:
        ...
        if add_pipe:
            ...
            for genome in ge:
                genome.fitness += 5
            pipes.append(Pipe(cog.WIN_WIDTH, pipe_img))

        for r in rem:
            pipes.remove(r)

        for bird in birds:
            if bird.y + bird.img.get_height() - 10 >= cog.FLOOR or bird.y < -50:
                nets.pop(birds.index(bird))
                ge.pop(birds.index(bird))
                birds.pop(birds.index(bird))
